use std::{collections::HashMap, convert::identity, iter::{Step, Sum}};
...
impl World {
    pub fn new(gfx: &Gfx, world_program: Program, world_generator: impl WorldGenerator + 'static) -> Self {
    ...
    pub fn extend_life_in_view(&mut self, center_voxel: VoxelPosition, distance: VoxelUnits) {
        self.world_chunks.extend_life_in_view(center_voxel, distance, &self.world_generator);
    }

    pub fn chunk_meshes<'a>(&'a mut self, gfx: &'a Gfx) -> impl Iterator<Item = (Location, &'a Mesh<DebugVertex>)> + 'a {
...
impl Chunks {
    ...
    fn extend_life(&mut self, chunk_position: ChunkPosition, now: Instant, world_generator: &DynWorldGenerator) {
        if let Some(chunk) = self.get_mut(chunk_position) {
            ...
            self.chunks.insert(
                chunk_position,
                Chunk::new(chunk_position.clone(), now, world_generator.as_ref().new_constructor()),
            );
        }
    }

    fn extend_life_in_view(&mut self, center_voxel: VoxelPosition, distance: VoxelUnits, world_generator: &DynWorldGenerator) {
        ...
        if EXTEND_IN_VIEW_IS_SPHERICAL {
            ...
            for chunk_position in self.chunk_positions_in_box((center_voxel - distance).to_chunk_position().0, (center_voxel + distance).to_chunk_position().0) {
                self.extend_life(chunk_position, now, world_generator);
            }
        }
    }

    fn chunk_positions_in_box(&self, min: ChunkPosition, max: ChunkPosition) -> impl Iterator<Item = ChunkPosition> {
        (min.z() ..= max.z()).flat_map(
            move |z|
            (min.y() ..= max.y()).flat_map(
                move |y|
                (min.x() ..= max.x()).map(
                    move |x| {
    ...
    fn chunk_positions_in_sphere(&self, center_chunk: ChunkPosition, distance: ChunkUnits) -> impl Iterator<Item = ChunkPosition> {
        let min = center_chunk - vector!(distance, distance, distance);
        let max = center_chunk + vector!(distance, distance, distance);
        self.chunk_positions_in_box(min, max).filter_map(move |chunk_position| {
    ...
    fn chunks_in_box(&self, min: ChunkPosition, max: ChunkPosition) -> impl Iterator<Item = (ChunkPosition, Option<&Chunk>)> {
        self.chunk_positions_in_box(min, max)
            .map(|chunk_position| (chunk_position, self.get(chunk_position)))
    }

    fn chunks_in_sphere(&self, center_chunk: ChunkPosition, distance: ChunkUnits) -> impl Iterator<Item = (ChunkPosition, Option<&Chunk>)> {
        self.chunk_positions_in_sphere(center_chunk, distance)
            .map(|chunk_position| (chunk_position, self.get(chunk_position)))
    }

    fn meshes<'a>(&'a mut self, gfx: &'a Gfx) -> impl Iterator<Item = (Location, &'a Mesh<DebugVertex>)> + 'a {
        self.chunks
            .iter_mut()
            .map(|(chunk_position, chunk)|
                {
                    ...
                    (
                        Location::new(
                            vector!(chunk_voxel_position.x().into_i64() as f32, chunk_voxel_position.y().into_i64() as f32, chunk_voxel_position.z().into_i64() as f32),
                            Quaternion::identity(),
                            Vector::one(),
                        ),
                        chunk.mesh(gfx),
...
impl Chunk {
    fn new(position: ChunkPosition, now: Instant, constructor: impl FnMut(VoxelPosition) -> Option<Voxel>) -> Self {
        Self {
    ...
    fn generate_mesh(&mut self, gfx: &Gfx) {
        ...
        for z in VoxelUnits(0) .. VOXELS_PER_CHUNK {
            for y in VoxelUnits(0) .. VOXELS_PER_CHUNK {
                ...
                let mut voxel_mesh_center = vector!(voxel_center.x() as f32, voxel_center.y() as f32, voxel_center.z() as f32);
...
impl Voxels {
    ...
    fn new(chunk_position: ChunkPosition, mut constructor: impl FnMut(VoxelPosition) -> Option<Voxel>) -> Self {
    ...
    fn position_to_index(&self, position: VoxelPosition) -> Option<usize> {
        ...
        let index = (position.x().0 * VOXELS_PER_CHUNK.0 * VOXELS_PER_CHUNK.0) + (position.y().0 * VOXELS_PER_CHUNK.0) + position.z().0;
        Some(index as usize)
    }
...
impl Into<usize> for ChunkUnits {
    fn into(self) -> usize {
        self.0.try_into().expect("ChunkUnits::into: i64 does not fit into a usize")
    }
}
...
impl Step for ChunkUnits {
    fn steps_between(start: &Self, end: &Self) -> Option<usize> {
        if start.0 < end.0 {
            Some((end.0 - start.0).try_into().expect("ChunkUnits::steps_between: i64 does not fit into a usize"))
        }
        else {
            None
        }
    ...
    fn forward_checked(start: Self, count: usize) -> Option<Self> {
        Some(Self(start.0 + i64::try_from(count).expect("ChunkUnits::forward_checked: usize does not fine into an i64")))
    }

    fn backward_checked(start: Self, count: usize) -> Option<Self> {
        Some(Self(start.0 - i64::try_from(count).expect("ChunkUnits::backward_checked: usize does not fine into an i64")))
    }
}
...
impl Step for VoxelUnits {
    fn steps_between(start: &Self, end: &Self) -> Option<usize> {
        if start.0 < end.0 {
            Some((end.0 - start.0).try_into().expect("VoxelUnits::steps_between: i64 does not fit into a usize"))
        }
        else {
            None
        }
    ...
    fn forward_checked(start: Self, count: usize) -> Option<Self> {
        Some(Self(start.0 + i64::try_from(count).expect("VoxelUnits::forward_checked: usize does not fine into an i64")))
    }

    fn backward_checked(start: Self, count: usize) -> Option<Self> {
        Some(Self(start.0 - i64::try_from(count).expect("VoxelUnits::backward_checked: usize does not fine into an i64")))
    }
}
...
impl VoxelPositionExt for VoxelPosition {
    fn to_chunk_position(&self) -> (ChunkPosition, VoxelPosition) {
        let float_chunk = vector!(self.x().0 as f64, self.y().0 as f64, self.z().0 as f64) / VOXELS_PER_CHUNK.0 as f64;
        let floored_float_chunk = float_chunk.map(|f| f.floor());
        (
            vector!((floored_float_chunk.x() as i64).into(), (floored_float_chunk.y() as i64).into(), (floored_float_chunk.z() as i64).into()),
            vector!(
                (((float_chunk.x() - floored_float_chunk.x()) * VOXELS_PER_CHUNK.0 as f64) as i64).into(),
                (((float_chunk.y() - floored_float_chunk.y()) * VOXELS_PER_CHUNK.0 as f64) as i64).into(),
                (((float_chunk.z() - floored_float_chunk.z()) * VOXELS_PER_CHUNK.0 as f64) as i64).into(),
            ).into()
        )
    }
...
impl DynWorldGenerator {
    pub fn new(world_generator: impl WorldGenerator + 'static) -> Self {
        Self { world_generator: Box::new(world_generator) }
    }
