...
const MAX_ELEVATION: f64 = 100.0;
const MIN_ELEVATION: f64 = -95.0;
...
impl BasicWorldGenerator {
    pub fn new(seed: u64) -> Self {
        let elevation_noise = Noise::<2>::new(seed, 5, 6.0, 5.0, 0.05);
        Self {
            seed,
            elevation_noise,
        }
...
impl WorldGenerator for BasicWorldGenerator {
    fn new_constructor(&self) -> Box<dyn FnMut(world::VoxelPosition) -> Option<Voxel>> {
        let seed = self.seed;
        let color_noise = Noise::<3>::new(seed.wrapping_mul(12345), 3, 2.0, 2.5, 0.5);
